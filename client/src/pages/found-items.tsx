    ...
    <div className="min-h-screen" style={{ backgroundColor: "hsl(210, 20%, 98%)" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold mb-2" style={{ color: "hsl(222, 47%, 11%)" }}>Found Items</h1>
            <p style={{ color: "hsl(215, 16%, 47%)" }}>Browse items that have been found and turned in</p>
          </div>
          <Button 
            onClick={() => setLocation("/report/found")} 
        ...
        <div className="flex justify-between items-center mb-4">
          <p className="text-sm" style={{ color: "hsl(215, 16%, 47%)" }}>
            Showing <span className="font-medium" style={{ color: "hsl(222, 47%, 11%)" }} data-testid="items-count">{items.length}</span> found items
          </p>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item) => (
              <ItemCard key={item.id} item={item} type="found" showUser />
            ))}
          </div>
        )}
